...
router.post("/", upload, async (req, res) => {
  try {
    const { home_title, home_description, home_more, about_title, about_description, about_link } = req.body;

    // Get the image paths from the uploaded files
    const home_image = req.files && req.files.home_image ? `/uploads/${req.files.home_image[0].filename}` : null;
    const about_image = req.files && req.files.about_image ? `/uploads/${req.files.about_image[0].filename}` : null;
